...
export const completionService = {
  ...
  /**
   * Notify assignees that a task with a due date was completed on time
   */
  async notifyDueDateCleared(
    taskId: string,
    task: { title: string; sectionId: string; dueDateValue: Date | null },
    notificationContext: NotificationContext
  ): Promise<void> {
    // Get workspace info
    const section = await database
      .selectFrom("section")
      .innerJoin("workspace", "workspace.id", "section.workspaceId")
      .select(["section.workspaceId", "workspace.name as workspaceName"])
      .where("section.id", "=", task.sectionId)
      .executeTakeFirst();
